...
pupil(N, C, S, Ind1)		 :- pupil(N, C, S, Ind), aux_index(Ind,Ind1,MI).
...
% auxiliary predicate used for each rule
% is true iff: Ind+1=Ind1 and Ind1<=MI (maximal index)
aux_index(Ind,Ind1,MI) :- index(Ind), #succ(Ind, Ind1), max_index(MI), Ind1 <= MI. 



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% We check if the number of atoms after applying T_C is the same as before apply it
...
fixpoint(LeastFixPoint) :- LeastFixPoint = #min{Ind : num_ot_atoms(Count, Ind), 
						   #succ(Ind, Ind1), num_ot_atoms(Count, Ind1)}.
